                          ...
                          uint2 * seeds)
{
    ...
    addSample(out, i, make_float4(sample.x,
                                  sample.y,
                                  sample.z,
                                  1.0));
    // Write the seed back
...
///////////////////////////////////////////////////////////////////////////////
// Kernel to do a median 3x3 filter to try to reduce noise
///////////////////////////////////////////////////////////////////////////////
__global__ void medianFilter(float * filtered, float * in, int width,
                             int height)
{
    ...
    // Get the median value for each channel in a 3x3 block around.
    // To do this, just do selection sort, as the number of elements is
    // small
    float sorted[3][FILTER_SIZE * FILTER_SIZE];

    // Fill in the block to look at
    for (int channel = 0; channel < 3; channel++)
    {
        for (int xOffSet = -(FILTER_SIZE - 1) /2;
                 xOffSet < (FILTER_SIZE - 1) / 2;
                 xOffSet++)
        {
            
            for (int yOffSet = -(FILTER_SIZE - 1) /2;
                     yOffSet < (FILTER_SIZE - 1) / 2;
                     yOffSet++)
            {
                // Figure out which place to write this pixel into
                int write = (xOffSet + (FILTER_SIZE - 1) / 2 + 
                             (yOffSet + (FILTER_SIZE - 1) / 2) * FILTER_SIZE);

                // Read the pixel the offsets refer to. If the desired pixel
                // would be out of bounds, fill it with 0 instead
                ...
                {
                    int read = (x + xOffSet + (y + yOffSet) * width) * 4;
                    sorted[0][write] = in[read];
                    sorted[1][write] = in[read + 1];
                    sorted[2][write] = in[read + 2];
                } 
            } 
        }
    } 
    ...
    {
        // Find the ith largest value in each channel
        float best[3] = {-1, -1, -1};
        int bestI[3];

        for (int j = i; j < FILTER_SIZE * FILTER_SIZE; j++)
        {
            if (sorted[0][j] > best[0])
            {
                best[0] = sorted[0][j];
                bestI[0] = j;
            }

            if (sorted[1][j] > best[1])
            {
                best[1] = sorted[1][j];
                bestI[1] = j;
            }

            if (sorted[2][j] > best[2])
            {
                best[2] = sorted[2][j];
                bestI[2] = j;
            }
        ...
        // Swap the ith largest channel value to its respective location
        float temp;

        temp = sorted[0][bestI[0]];
        sorted[0][bestI[0]] = sorted[0][i];
        sorted[0][i] = temp;

        temp = sorted[1][bestI[1]];
        sorted[1][bestI[1]] = sorted[1][i];
        sorted[1][i] = temp;

        temp = sorted[2][bestI[2]];
        sorted[2][bestI[2]] = sorted[2][i];
        sorted[2][i] = temp;
    }

    // Set the channel values to the medians
    
    filtered[idx * 4] = sorted[0][FILTER_SIZE * FILTER_SIZE / 2 ];
    filtered[idx * 4 + 1] = sorted[1][FILTER_SIZE * FILTER_SIZE / 2];
    filtered[idx * 4 + 2] = sorted[2][FILTER_SIZE * FILTER_SIZE / 2];
}
